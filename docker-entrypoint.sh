...
if [ ${HASURA_GRAPHQL_MIGRATIONS_DATABASE_ENV_VAR} ]; then
    log "This is the env $HASURA_GRAPHQL_MIGRATIONS_DATABASE_ENV_VAR"
    HASURA_GRAPHQL_MIGRATIONS_DATABASE_URL=$(echo \$$HASURA_GRAPHQL_MIGRATIONS_DATABASE_ENV_VAR) > /dev/null 2>&1
